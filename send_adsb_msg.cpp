//based on c_uart_interface_example/send_quad_commands.cpp
...
int open_port(const char* port)
{
	...
	if (fd == -1)
	{
		/* Could not open the port. */
	        cout<<"computer to pixhawk port open fails"<<endl;
		return(-1);
	}
	else
	{
 ...
 * This funciton sends ADS-B data to the pixhawk
 *
 * This function sends data out
 * and forwards the data once received.
 */
int adsb_serial_send(int serial_fd)
{
    int fd = serial_fd;
